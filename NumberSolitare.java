...
public class NumberSolitare {
	int solution(int[] A) {
		...
		maxValue[0] = A[0];
		// Represents where the dice is located
		for (int i = 0; i < maxValue.length; i++) {
			// Represents the dice rolls and maximal values obtainable
			for (int j = 1; j <= maxDice && i + j < maxValue.length; j++) {
				maxValue[i + j] = Math.max(maxValue[i + j], maxValue[i] + A[i + j]);
			}
		}
